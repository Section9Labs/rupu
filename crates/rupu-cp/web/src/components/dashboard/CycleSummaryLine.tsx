...
export function CycleSummaryLine({
  ...
  return (
    <div className="flex items-center justify-between text-sm text-ink-dim">
      <span>
        <span className="font-medium text-ink">{cycles.total}</span> cycles ·{' '}
        <span className="text-ink">{cycles.clean === null ? '—' : cycles.clean}</span>{' '}
        clean ·{' '}
      ...
      <Link to="/runs" className="text-ink-mute hover:text-ink">
        see all →
      </Link>
    </div>
  );
