...
/// [`run_and_unit_ids`] names, each resolved through [`resolve_ledger_path`]
...
/// `resolve_ledger_path` reads from when the workspace never got its own
/// `.rupu/netflow/` — see that function's doc) and whatever `store`
