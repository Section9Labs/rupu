...
use rupu_cli::paths::{
    ...
    autoflow_wakes_dir, autoflow_workers_dir, autoflow_worktrees_dir, autoflows_dir, global_dir,
    project_root_for, repos_dir, transcripts_dir,
};
...
fn state_dirs_live_under_global_root() {
    ...
    assert_eq!(
        autoflow_workers_dir(tmp_global.path()),
        tmp_global.path().join("autoflows/workers")
    );
    assert_eq!(
