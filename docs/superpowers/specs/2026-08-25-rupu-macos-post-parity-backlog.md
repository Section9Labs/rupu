...
| 6 | Overview Customize block drag/reorder edit mode | Phase 4 | Umbrella §8 Phase 4 row | HANDOFF.md specifies drag/reorder for the Customize visibility menu; Phase 4 shipped show/hide persistence only. ~~Explicitly deferred to the post-phases UI-redesign pass.~~ **LANDED** — redesign pass (cb03ebc4: reorder sheet + persisted order; needsYou is two-position around the merged chart group, disclosed in the editor). |
...
| 18 | ~~Situation Room: follow/pin + fresh-highlight interactions~~ **LANDED** | Phase 6 | Umbrella §8 Phase 6 row | The fullscreen wall ports the web's cards/roster/vitals algorithms; the follow/pin and fresh-highlight interaction affordances were parked → redesign pass at the Phase 7 checkpoint, and landed in that pass's Task 4 (`feat(macos): Situation Room — follow/pin scroll contract + fresh-arrival highlight`, commit `43f91ca6`). Row kept (not deleted) since `docs/superpowers/specs/2026-08-25-rupu-macos-redesign-pass-design.md` cites it by number as an input row. |
...
| 23 | Visual nit: pill shape | Design-alignment (Plan 1/2, PR #598/#599) | `macos-ui-redesign-deferred` session memory (matt, 2026-08-24) | ~~SwiftUI `Capsule` pill shape vs. the web's rounded-4 corner radius.~~ **LANDED** — redesign pass (b961644e). |
| 24 | Visual nit: tinted-vs-flat status pills | Design-alignment (Plan 1/2, PR #598/#599) | `macos-ui-redesign-deferred` session memory (matt, 2026-08-24) | ~~`pending`/`cancelled`/`skipped` render tinted in the app; the web renders them flat-neutral.~~ **LANDED** — redesign pass (b961644e). |
| 25 | Visual nit: brand header badge | Design-alignment (Plan 1/2, PR #598/#599) | `macos-ui-redesign-deferred` session memory (matt, 2026-08-24) | ~~Flagged during the #598/#599 checkpoint review.~~ **LANDED** — redesign pass (593df7d8: audit pinned it to the missing badge + "Control Plane" subtitle; Brand.tsx-matched). |
| 26 | Visual nit: "Awaiting approval" wording | Design-alignment (Plan 1/2, PR #598/#599) | `macos-ui-redesign-deferred` session memory (matt, 2026-08-24) | ~~Copy nit on the gate-pending state's label.~~ **LANDED** — redesign pass (593df7d8: audit pinned it to Title-Case/short-form drift vs the canonical "Awaiting approval"). |
| 27 | Visual nit: scope-select control thinness | Design-alignment (Plan 1/2, PR #598/#599) | `macos-ui-redesign-deferred` session memory (matt, 2026-08-24) | The top-bar project-scope picker reads visually thin next to the web equivalent. PR #612 (merged 2026-08-25) reworked the toolbar — whether this nit is resolved needs matt's eye at the redesign-pass checkpoint. |
