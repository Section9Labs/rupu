...
| 4 Dashboard | **Delivered** (spec: `docs/superpowers/specs/2026-08-24-rupu-macos-phase-4-dashboard-design.md`): v2-intent Overview — needs-you queue (client-side aggregate; `GET /api/attention` deferred, tracked), instrument/freshness/fleet strips, stacked-area trend charts, Customize block-visibility persistence (`overview.widgets`); HANDOFF's drag/reorder edit mode deferred to the post-phases UI-redesign pass | `dashboard` |
| 5 Breadth | **Plan A delivered** (spec: `docs/superpowers/specs/2026-08-24-rupu-macos-phase-5-breadth-design.md`): Projects (tabbed detail; Code/Config tabs → Phase 6), Fleet (cards + workers + remove; add-host forms deferred → CLI), Library (definition tabs + read-only detail; editors + "Used by" → post-parity backlog), and the **scope-aware launch fix** (server + client; the Phase 3 tracked gap — local-only, Global enforced-or-400). **Plan B delivered**: Security (global findings with `declared_by` surface-split navigation — agent/session-surface rows non-navigating pending richer row data, tracked; coverage summary + detail overview/catalog; audit/gap/runs/diff + templates deferred → post-parity backlog), Usage (spend chart + pivot breakdown + outliers over `/api/usage`+`/runs`+`/outliers`; `usage-timeline` endpoint dormant, matching web; custom drag-select windows deferred). **Phase 5 complete — every umbrella screen is real.** | ALL modules shipped: `projects`, `coverage`, `findings` (global), `agents`/`workflows`/`autoflows` (definitions), `hosts`, `workers`, `usage`, `usage_outliers`; `repos` deferred (tracked, no consumer); `repo_scope` internal (n/a) |
| 6 Ambient | **Delivered** (spec: `docs/superpowers/specs/2026-08-25-rupu-macos-phase-6-ambient-design.md`). Plan A: four-tab Settings (Config = Effective/provenance + raw TOML editors + policy locks; typed form tabs deferred → post-parity), UNUserNotificationCenter notifications (per-step gate dedup; critical-finding alerts deferred — not on the event wire), menu bar extra (local-only, disclosed). Plan B: Situation Room fullscreen scene (web cards/roster/vitals algorithms ported + table-tested; follow/pin + fresh-highlight parked → redesign pass), project Code tab + transcript ast_grep source/CST previews (web mounts ast_grep only — verified), autoflow claims Runs/Claims toggle. ~~⌘K palette~~ landed earlier (design-alignment Plan 2). **Phase 6 complete.** | `config` (read + write) shipped; `source`, `code`, `autoflow_claims` shipped; `workspace` intentionally no UI (host-to-host wire protocol, no web UI either — n/a by design); `fs` deferred (native NSOpenPanel covers local; remote browsing has no consumer); `fleet_inventory` already consumed via `GET /api/dashboard`'s fleet field (Phase 4) |
| 7 Ship | **Delivered** (spec: `docs/superpowers/specs/2026-08-25-rupu-macos-phase-7-ship-design.md`): `make macos-release` (version-stamped, hardened runtime, ad-hoc local signing) + `scripts/package-app-dmg.sh` (`hdiutil`-based, no third-party tooling); `release.yml` gains a `macos-app` job (same tag cadence as the CLI, imports the existing Developer ID cert, signs, notarizes, staples, packages `rupu-app-darwin-arm64.dmg`/`.zip`/`.sha256`, uploaded alongside the CLI assets). Self-update (Sparkle/appcast) and sandboxing recorded out of scope this phase (post-parity backlog spec). **Post-parity backlog seeded**: `docs/superpowers/specs/2026-08-25-rupu-macos-post-parity-backlog.md` collects every deferred-tracked item from Phases 2–6 plus the parked visual nits from the redesign-pass memory. **Program complete — rupu.app rides the CLI's release cadence; the lane's first live sign→notarize→staple run is the checkpoint beta cut (spec §6), unproven until then.** | — |
