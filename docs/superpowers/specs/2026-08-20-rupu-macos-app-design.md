...
| 5 Breadth | **Plan A delivered** (spec: `docs/superpowers/specs/2026-08-24-rupu-macos-phase-5-breadth-design.md`): Projects (tabbed detail; Code/Config tabs → Phase 6), Fleet (cards + workers + remove; add-host forms deferred → CLI), Library (definition tabs + read-only detail; editors + "Used by" → post-parity backlog), and the **scope-aware launch fix** (server + client; the Phase 3 tracked gap — local-only, Global enforced-or-400). **Plan B pending**: Security (findings/coverage; audit/gap/diff + templates deferred), Usage | `projects`, `agents`/`workflows`/`autoflows` (definitions), `hosts`, `workers` shipped; `coverage`, `findings` (global — model shipped, ws-scoped call shipped in Plan A), `usage`, `usage_outliers` in Plan B; `repos` deferred (tracked, no consumer); `repo_scope` internal (n/a) |
