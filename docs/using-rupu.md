...
`rupu autoflow serve` now opens an interactive operator console by default when attached to a terminal. The live screen keeps an issue list and a selected-issue detail pane on screen together, so you can see which issues are active, blocked, queued, or complete and inspect the selected issue’s workflow timeline without leaving the worker. The selected issue pane shows repo/branch/PR routing, run/usage/change summaries, and the workflow stage canvas with live transcript rows nested under the current step. Use `Tab` to switch focus between the issue list and detail pane, `Enter` to pin/unpin the current issue, `Space` to jump back to the hottest issue, `f` to cycle `focused → compact → full`, and the usual scroll keys (`↑/↓`, `j/k`, `PgUp/PgDn`, `g`, `G`) in the detail pane. Use `--quiet` when you want the old daemon-style behavior with no live screen. `rupu autoflow monitor --watch` now uses the same operator-console layout in a read-only mode, so you can inspect the same issue list, selected issue timeline, and queue pressure without running a worker. Plain `rupu autoflow monitor` in a terminal now renders the same issue-list + selected-issue snapshot instead of the old summary frame, and `--view focused|compact|full` applies there too. `rupu autoflow history --watch` uses the same retained layout, but the selected issue detail pane shows the durable history timeline for that issue, optionally alongside the current workflow canvas when the issue is still active. Plain `rupu autoflow history` in a terminal now renders the same issue-list + selected-issue snapshot instead of the old flat history table, and `--view focused|compact|full` applies there too. Use `rupu --format json autoflow monitor` or `rupu --format json autoflow history` when you want structured output for automation.
