...
- 🚧 **2c — interactive sessions**: send a message to a live session — via a `SessionSender` port whose `cp serve` adapter shells `rupu session send <id> <prompt> --detach` (subprocess, same pattern as the 2b launcher; reproducing the queue write is unsafe because of the private `SessionRecord` bookkeeping). **Send only** in v1. Follow-ups: session **start / stop / archive / delete** lifecycle control (also subprocess-able: `rupu session stop|archive|restore|delete`), and an **inline live-transcript pane** in SessionDetail (today the composer relies on the 5s-polled Turn Runs list + the `/transcript` stream route).
