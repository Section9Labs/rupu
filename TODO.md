...
- 🚧 Native tracker state events are in progress. Shipped so far: normalized native state aliases, Linear webhook ingress, Linear team polling, Jira webhook ingress, Jira project polling, GitHub Projects v2 webhook state mapping with optional GitHub-backed payload hydration for sparse Projects deliveries, a generic trigger-source model that can represent repo-backed and tracker-project sources, and tracker-native autoflow ownership. Remaining work: GitHub Projects polling if GitHub exposes a stable source for it, pull-request or draft-issue alias expansion for GitHub Projects items, and future non-repo tracker families beyond Linear/Jira/GitHub Projects. Event design: [`docs/superpowers/specs/2026-05-10-rupu-native-tracker-state-events-design.md`](docs/superpowers/specs/2026-05-10-rupu-native-tracker-state-events-design.md). Ownership design: [`docs/superpowers/specs/2026-05-10-rupu-tracker-native-autoflow-ownership-design.md`](docs/superpowers/specs/2026-05-10-rupu-tracker-native-autoflow-ownership-design.md). Ownership plan: [`docs/superpowers/plans/2026-05-10-rupu-tracker-native-autoflow-ownership-plan-1.md`](docs/superpowers/plans/2026-05-10-rupu-tracker-native-autoflow-ownership-plan-1.md).
...
- ✅ **2c — interactive sessions (send)**: send a message to a live session — `SessionSender` port + `cp serve` subprocess adapter (`rupu session send <id> <prompt> --detach`) + composer (#364). Follow-ups: session **start / stop / archive / delete** lifecycle control (subprocess-able: `rupu session stop|archive|restore|delete`).
- 🚧 **2e — full session chat**: the Session page becomes an interactive conversation (prior turns + live streaming turn + composer), matching the CLI's attached session. Assembles the existing `TranscriptPanel`/SSE renderer + the 2c send; one backend add (`GET /api/sessions/:id/runs` exposing per-turn `prompt`). Chat is the primary view; recent-turns + "load older". Deferred: char-by-char typing animation, inline approval-gate handling in chat, virtualized history.
