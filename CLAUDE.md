...
3. **Module map:** `RupuAPI` (CPClient, CPEvent, SSE `EventStreamClient`) · `RupuBackend` (discovery, version gate, Keychain, `EmbeddedServer` attach-or-spawn, `HealthMonitor`) · `RupuStore` (`AppModel`, `BackendController`, `PagedSnapshot`/`LiveReducer`/`StreamLifecycle` primitives, `PendingAction`, `LauncherStore`) · `RupuDesign` (programmatic design tokens — `HANDOFF.md` is the visual contract) · `RupuShell` (views) · `RupuActivity` (Activity table — kind/status filters, live tail, new-run pill, inline gate Approve/Reject) · `RupuRunDetail` (run detail — step graph, transcript feed, netflow/findings rails, mutations — and session detail with send/archive/restore) · `RupuLauncher` (the "+ New run" sheet — definition picker, prompt/inputs form, mode + host chips, client-side fan-out). Later phases add one module per screen.
